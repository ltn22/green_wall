...
REPORT_PERIOD = 10 # send a frame every 60 sample (1 hour)
...
moisture_offset = 3.5*REPORT_PERIOD // 4
...
while True:
    ...
    #send_coap_message (s, destination, "TPH", [t, p, h])
    ...
    if len(cbor.dumps(mois_ts)) > MTU-coap_header_size:
        answer = send_coap_message(s, destination, "moisture", mois_ts[:-1])
        mois_ts=[mo]
    ...
    if len(mois_ts) + moisture_offset > REPORT_PERIOD:
        answer = send_coap_message(s, destination, "moisture", mois_ts)
        answer = send_coap_message(s_lora, ("LORAWAN", 0), "moisture", mois_ts)
        mois_ts = []
        moisture_offset = 0
